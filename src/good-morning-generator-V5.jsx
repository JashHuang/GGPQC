...
import './good-morning-generator-V5.css';
...
const normalizeWisdomPath = (path) => {
  ...
  // 不要強制加上開頭的 /，因為在 Electron 中這會指向根目錄
...
const GoodMorningGeneratorV5 = () => {
  ...
  const saveImage = () => {
    ...
    requestAnimationFrame(() => {
      requestAnimationFrame(() => {
        ...
        link.download = `good-morning-v5-${Date.now()}.jpg`;
  ...
  return (
    <div className="gm5-page">
      <div className="gm5-shell">
        <header className="gm5-header">
          <div>
            <h1>Good Morning Generator V5</h1>
            <p>行動版工具集中於頂部與底部，桌面版延續 V4 編輯流程</p>
          </div>
          <div className="gm5-header-actions">
            <details className="gm5-user-menu">
              <summary className="gm5-btn gm5-btn-soft">使用者設定</summary>
              <div className="gm5-user-panel">
                ...
                <div className="gm5-row">
                  <input
                    ...
                    placeholder="新增使用者名稱"
                  />
                  <button className="gm5-btn gm5-btn-soft" onClick={handleCreateUser}>新增/切換</button>
                </div>
                <p className="gm5-meta">每位使用者會記住字型與顏色設定。</p>
              </div>
            </details>
            <button className="gm5-btn gm5-btn-soft" onClick={undo} disabled={!canUndo}><Undo /> 復原</button>
            <button className="gm5-btn gm5-btn-soft" onClick={redo} disabled={!canRedo}><Redo /> 重做</button>
            <button className="gm5-btn gm5-btn-primary" onClick={saveImage}><Download /> 匯出 JPG</button>
          </div>
        </header>
        ...
        <main className="gm5-main">
          <aside className="gm5-sidebar">
            <section className="gm5-card">
              ...
              <div className="gm5-row">
                <button className="gm5-btn gm5-btn-primary" onClick={() => addTextBlock('greeting')}>新增問候語框</button>
                <button className="gm5-btn gm5-btn-soft" onClick={() => addTextBlock('wisdom')}>新增智慧語框</button>
              </div>
            </section>
            ...
            <section className="gm5-card">
              ...
              <div className="gm5-row">
                <button className="gm5-btn gm5-btn-soft" onClick={generateRandomQuote} disabled={!wisdomLoaded}>隨機語錄</button>
              </div>
              <textarea
                className="gm5-input gm5-textarea"
                value={wisdomText}
                onChange={(e) => {
                  const v = e.target.value;
                  setWisdomText(v);
            ...
            <section className="gm5-card">
              ...
              <div className="gm5-layers">
                {orderedLayers.map((block) => {
                  const isSelected = selectionSet.has(block.id);
                  return (
                    <div key={block.id} className={`gm5-layer ${isSelected ? 'is-selected' : ''}`}>
                      <button
              ...
              <button className="gm5-btn gm5-btn-danger" disabled={!selectedIds.length} onClick={deleteSelected}>刪除選取區塊 ({selectedIds.length})</button>
            </section>

          </aside>

          <section className="gm5-preview">
            <div className="gm5-mobile-topbar">
              <button className="gm5-btn gm5-btn-soft" onClick={undo} disabled={!canUndo}><Undo /> 復原</button>
              <button className="gm5-btn gm5-btn-soft" onClick={redo} disabled={!canRedo}><Redo /> 重做</button>
              <button className="gm5-btn gm5-btn-primary" onClick={saveImage}><Download /> 匯出</button>
            </div>
            <div className="gm5-mobile-secondary-tools">
              <button className="gm5-btn gm5-btn-primary" onClick={() => addTextBlock('greeting')}>+ 問候框</button>
              <button className="gm5-btn gm5-btn-soft" onClick={() => addTextBlock('wisdom')}>+ 智慧框</button>
              <button className="gm5-btn gm5-btn-danger" disabled={!selectedIds.length} onClick={deleteSelected}>刪除 ({selectedIds.length})</button>
            </div>

            <div className="gm5-toolbar">
              <div className="gm5-row">
                <button className="gm5-btn gm5-btn-soft" onClick={() => setZoom((z) => clamp(z - 10, minZoom, maxZoom))}>-</button>
                <button className="gm5-btn gm5-btn-soft" onClick={() => setZoom(100)}>{zoom}%</button>
                <button className="gm5-btn gm5-btn-soft" onClick={() => setZoom((z) => clamp(z + 10, minZoom, maxZoom))}>+</button>
              </div>
              <div className="gm5-row">
                <button className={`gm5-btn ${showGrid ? 'gm5-btn-primary' : 'gm5-btn-soft'}`} onClick={() => setShowGrid((v) => !v)}>網格</button>
                <button className={`gm5-btn ${showRuler ? 'gm5-btn-primary' : 'gm5-btn-soft'}`} onClick={() => setShowRuler((v) => !v)}>尺規</button>
                <span className="gm5-meta">畫布 {canvasSize.width} x {canvasSize.height}</span>
              </div>
            </div>

            <div className="gm5-canvas-wrap" ref={wrapRef}>
              <div className="gm5-canvas-stage" style={{ width: canvasDisplayWidth, height: canvasDisplayHeight }}>
                {showGrid && <div className="gm5-grid" />}
                {showRuler && (
                  <>
                    <div className="gm5-ruler gm5-ruler-top" />
                    <div className="gm5-ruler gm5-ruler-left" />
                  </>
                )}
                <canvas
                  ref={canvasRef}
            ...
            <p className="gm5-tip">快捷鍵：Ctrl/Cmd+S 匯出、Ctrl/Cmd+Z 復原、Shift+Z/Ctrl+Y 重做、Delete 刪除、方向鍵微調</p>

            <div className="gm5-mobile-bottom-tools">
              <div className="gm5-mobile-bottom-main">
                <button className="gm5-btn gm5-btn-soft" onClick={() => setZoom((z) => clamp(z - 10, minZoom, maxZoom))}>-</button>
                <button className="gm5-btn gm5-btn-soft" onClick={() => setZoom(100)}>{zoom}%</button>
                <button className="gm5-btn gm5-btn-soft" onClick={() => setZoom((z) => clamp(z + 10, minZoom, maxZoom))}>+</button>
                <button className={`gm5-btn ${showGrid ? 'gm5-btn-primary' : 'gm5-btn-soft'}`} onClick={() => setShowGrid((v) => !v)}>網格</button>
              ...
              {mobilePanel && (
                <div className="gm5-mobile-drawer">
                  {mobilePanel === 'assets' && (
                  ...
                  {mobilePanel === 'settings' && (
                    <div className="gm5-mobile-panel">
                      <h3 className="gm5-mobile-panel-title">使用者設定</h3>
                      <select
                        className="gm5-input"
                        value={activeUser}
                        onChange={(e) => handleSwitchUser(e.target.value)}
              ...
              <div className="gm5-mobile-tabs">
                <button
                  className={`gm5-btn ${mobilePanel === 'assets' ? 'gm5-btn-primary' : 'gm5-btn-soft'}`}
                  onClick={() => setMobilePanel((p) => (p === 'assets' ? null : 'assets'))}
                >
                  素材
                </button>
                <button
                  className={`gm5-btn ${mobilePanel === 'greeting' ? 'gm5-btn-primary' : 'gm5-btn-soft'}`}
                  onClick={() => setMobilePanel((p) => (p === 'greeting' ? null : 'greeting'))}
                >
                  問候
                </button>
                <button
                  className={`gm5-btn ${mobilePanel === 'wisdom' ? 'gm5-btn-primary' : 'gm5-btn-soft'}`}
                  onClick={() => setMobilePanel((p) => (p === 'wisdom' ? null : 'wisdom'))}
                >
                  智慧
                </button>
                <button
                  className={`gm5-btn ${mobilePanel === 'layers' ? 'gm5-btn-primary' : 'gm5-btn-soft'}`}
                  onClick={() => setMobilePanel((p) => (p === 'layers' ? null : 'layers'))}
                >
                  圖層
                </button>
                <button
                  className={`gm5-btn ${mobilePanel === 'settings' ? 'gm5-btn-primary' : 'gm5-btn-soft'}`}
